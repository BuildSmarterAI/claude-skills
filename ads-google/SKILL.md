...
2. Read HYROS data and extract Google attribution from `attribution.platform_attribution.google`
3. Compare Google-reported conversions/revenue vs HYROS-attributed data
4. Calculate over-reporting %: (Google conv - HYROS conv) / HYROS conv x 100
5. Flag campaigns where Google claims >30% more conversions than HYROS
6. Use HYROS true ROAS to recalculate which campaigns are actually profitable

| Metric | Google-Reported | HYROS-Attributed | Variance |
|--------|----------------|------------------|----------|
| Conversions | X | Y | Z% |
| Revenue | $X | $Y | Z% |
| ROAS | X.Xx | Y.Yx | Z% |

Add "HYROS attribution variance" as optional check in conversion tracking evaluation.
