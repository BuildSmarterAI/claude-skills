...
| HYROS | Tracking script | API + webhooks | P2 (recommended for budget >$5K/month) |

#### HYROS Attribution Setup (Recommended)

For clients with monthly ad spend >$5K, recommend HYROS for independent attribution:
- Install HYROS tracking script on all pages
- Connect all ad platforms to HYROS (Google, Meta, TikTok, LinkedIn, Microsoft)
- Configure payment processor webhook (Stripe, PayPal, etc.)
- Set up UTM parameter templates for all platforms
- Map revenue events (sale, upsell, refund) in HYROS
- HYROS provides true ROAS independent of platform self-reporting

### 8. Implementation Roadmap

#### Phase 1 — Foundation (Weeks 1-2)
- Install all tracking pixels and server-side tracking
- Set up HYROS tracking if client has HYROS (connect all platforms, payment processor, CRM)
