...
## HYROS Landing Page Attribution (Optional)

When `output/hyros_data.json` is available, cross-reference landing page data with HYROS attribution:

1. Compare landing page conversion tracking with HYROS revenue data
2. Identify landing pages where platform reports conversions but HYROS shows no attributed revenue (tracking leak)
3. Verify UTM parameter capture by comparing HYROS source tags with landing page UTMs
4. Flag landing pages with high platform-reported CVR but low HYROS-attributed revenue (false positive conversions)

### Tracking Leak Detection

| Landing Page | Platform Conversions | HYROS Revenue | Status |
|-------------|---------------------|---------------|--------|
| /page-a | 50 | $5,000 | Healthy |
| /page-b | 30 | $0 | Tracking leak |
| /page-c | 20 | $8,000 | Under-reported |

Common causes of tracking leaks:
- UTM parameters stripped on redirect
- Click IDs (gclid, fbclid, ttclid) not passed through form submission
- Thank you page fires platform pixel but not HYROS tracking
- Cross-domain tracking broken between ad click and conversion page
